...
yum -y install rubygems ruby-devel rubygem-stomp     #默认yum安装
...
#http://rpm.pbone.net 
#ftp://ftp.muug.mb.ca/mirror/fedora/epel/6/i386/rubygem-rest-client-1.6.1-2.el6.noarch.rpm
rpm -ivh http://download.fedora.redhat.com/pub/epel/6/x86_64/rubygem-json-1.4.3-3.el6.x86_64.rpm 
#ftp://ftp.sunet.se/pub/Linux/distributions/fedora/epel/testing/6/x86_64/rubygem-json-1.4.3-3.el6.x86_64.rpm
rpm -ivh http://download.fedora.redhat.com/pub/epel/6/x86_64/rubygem-mime-types-1.16-3.el6.noarch.rpm 
#ftp://mirror.switch.ch/pool/4/mirror/epel/6/ppc64/rubygem-mime-types-1.16-3.el6.noarch.rpm
 
# Installation of stack gems  一直出 ERROR: While executing gem ... (Zlib::BufError)，错误 只要先运行 gem update --system 即可 
#net-ping  用rvm升级 ruby   rvm install 1.9.3  rvm use 1.9.3 (查看版本如没变 则rvm use 1.9.3 --default)
gem install --no-rdoc --no-ri puppet passenger rack mysql net-ping 
 
 #rvm gemset delete global  rvm remove 1.9.2 rvm remove 2.0.0 用系统自带ruby安装成功 
#gem install --no-rdoc --no-ri -v 3.0.10 rails activerecord   
gem install --no-rdoc --no-ri -v  rails activerecord   
...
cp  /usr/lib/ruby/gems/1.8/gems/puppet-3.6.2/ext/rack/config.ru  /usr/share/puppet/rack/puppetmasterd/config.ru 
